...
print(f"    故障/正常比例: {binary_dist[1]/binary_dist[0]:.2f}")
...
X_selected = combined_df[selected_features + ['binary_label', 'condition']]
X_selected.to_csv(f'{data_dir}/binary_processed_data.csv', index=False)

# 5. 划分训练集和测试集 (7:3)
print("\n[4] 划分训练集和测试集 (7:3)...")
X_binary = combined_df[selected_features].values
y_binary = combined_df['binary_label'].values
...
ax.set_title('Confusion Matrix - Binary Classification\n(Random Forest: Normal vs Fault)', fontsize=14)
plt.tight_layout()
output_path = f'{data_dir}/binary_confusion_matrix.png'
...
feature_importance = rf_binary.feature_importances_
imp_df = pd.DataFrame({
    'feature': selected_features,
    'importance': feature_importance
}).sort_values('importance', ascending=True)
ax2.barh(imp_df['feature'], imp_df['importance'], color='steelblue', edgecolor='black')
ax2.set_xlabel('Feature Importance', fontsize=12)
ax2.set_ylabel('Feature', fontsize=12)
ax2.set_title('Binary Classification Feature Importance (Random Forest)\nNormal(0%) vs Fault(5%~50%)', fontsize=14)
plt.tight_layout()
importance_path = f'{data_dir}/binary_feature_importance.png'
plt.savefig(importance_path, dpi=150, bbox_inches='tight', facecolor='white')
print(f"特征重要性图已保存至: {importance_path}")

# 12. 保存结果
print("\n[12] 保存结果...")
results_path = f'{data_dir}/binary_evaluation_results.txt'
with open(results_path, 'w', encoding='utf-8') as f:
    f.write("=" * 70 + "\n")
    f.write("二分类模型评估结果 - 热泵冷却系统故障检测\n")
...
pred_results.to_csv(f'{data_dir}/binary_prediction_results.csv', index=False)
print(f"预测结果已保存至: {data_dir}/binary_prediction_results.csv")

pd.DataFrame({'selected_features': selected_features}).to_csv(f'{data_dir}/binary_selected_features.csv', index=False)
print(f"选中特征已保存至: {data_dir}/binary_selected_features.csv")
