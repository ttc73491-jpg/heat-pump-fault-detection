...
output_dir = r'C:\Users\ccc\Desktop\algorithm\16_DNN_新特征\output'
os.makedirs(output_dir, exist_ok=True)

# ========== 特征配置 ==========
selected_path = r'C:\Users\ccc\Desktop\algorithm\13_新特征值_特征提取\output\selected_features.csv'
...
def phase1_architecture_sweep():
    print("=" * 70)
    print("Phase 1: DNN 架构扫描 — 新39特征数据集")
    print("=" * 70)
    ...
    # 与全量数据原始架构扫描对比
    print(f"\n{'='*70}")
    print("新特征 vs 原始数据 架构扫描对比")
    print(f"{'='*70}")
    print(f"{'架构':<22} {'新特征准确率':<14} {'原始准确率':<12} {'变化':<10}")
    print("-" * 58)
    original_acc = {
...
def save_sweep_results(arch_name, results, history, train_time, stopped_epoch, y_test):
    ...
    with open(txt_path, 'w', encoding='utf-8') as f:
        f.write("=" * 70 + "\n")
        f.write(f"DNN 新特征架构扫描 — {arch_name}\n")
        f.write("=" * 70 + "\n\n")
    ...
    ax1.set_title(f'New Features {arch_name} — Accuracy')
    ax1.legend(); ax1.grid(True, alpha=0.3)
    ax2.plot(history.history['loss'], label='Train')
    ax2.plot(history.history['val_loss'], label='Val')
    ax2.set_xlabel('Epoch'); ax2.set_ylabel('Loss')
    ax2.set_title(f'New Features {arch_name} — Loss')
...
def plot_comparison_chart(summary_df):
    ...
    ax.set_title('DNN Architecture Sweep — New Features\n'
...
def phase2_hierarchical(best_arch_name, best_hidden_units):
    ...
    plot_cm_basic(cm_s1, SEVERITY_NAMES,
                  f'DNN {best_arch_name} Stage 1: 4-Class ({N_FEATURES} features)',
                  'dnn_stage1_cm.png')
    ...
    for group_id in range(4):
        ...
        plot_cm_basic(cm_sub, group_label_names,
                      f'DNN {best_arch_name} {GROUPS[group_id]["name"]} Group',
...
def main():
    ...
    # ==================== 最终对比 ====================
    print("\n\n" + "=" * 70)
    print("最终对比: 新特征DNN vs 历史最佳 vs 原始DNN")
    print("=" * 70)

    # 找到扁平最佳
    best_flat_acc = best['accuracy'] * 100

    print(f"\n{'算法':<30} {'Stage 1':>10} {'分层10分类':>12} {'备注':>20}")
    print("-" * 75)
    print(f"{'DNN 新特征 (扁平最佳)':<30} {'—':>10} {best_flat_acc:>11.2f}% {'架构: '+best_name:>20}")
    print(f"{'DNN 新特征 (分层)':<30} {acc_s1*100:>9.2f}% {overall_acc:>11.2f}% {'架构: '+best_name:>20}")
    print(f"{'RF 新特征':<30} {'(见RF结果)':>10}")
    print(f"{'SVM 新特征':<30} {'(见SVM结果)':>10}")
    print(f"{'RF 瞬态 (原最佳)':<30} {'70.89%':>10} {'69.59%':>12}")
    print(f"{'DNN 瞬态分层':<30} {'56.33%':>10} {'61.74%':>12}")
    print(f"{'RF 原始全量':<30} {'45.47%':>10} {'58.87%':>12}")
    print(f"{'DNN 原始分层':<30} {'45.54%':>10} {'56.06%':>12}")

    # 保存最终结果
    with open(os.path.join(output_dir, 'dnn_new_features_final_results.txt'), 'w', encoding='utf-8') as f:
        f.write("=" * 70 + "\n")
        f.write(f"DNN 新特征(39→{N_FEATURES}) 最终结果\n")
        f.write("=" * 70 + "\n\n")
        ...
        f.write(f"\nComparison:\n")
        f.write(f"  DNN New Features Flat:    {best_flat_acc:.2f}%\n")
        f.write(f"  DNN New Features Hier:    {overall_acc:.2f}%\n")
        f.write(f"  RF Transient (best ever): 69.59%\n")
        f.write(f"  DNN Original Hier:        56.06%\n")
        f.write(f"  RF Original Hier:         58.87%\n")

    print(f"\n结果已保存至: {output_dir}/dnn_new_features_final_results.txt")
    print("=" * 70)
    print("DNN 新特征实验完成!")
    print("=" * 70)
