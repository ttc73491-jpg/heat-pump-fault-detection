...
output_dir = r'C:\Users\ccc\Desktop\algorithm\14_随机森林_新特征\output'
...
# ==================== 数据加载 ====================
print("=" * 70)
print("随机森林 — 新39特征数据集 (15 selected features)")
print("=" * 70)
...
# ==================== 扁平10分类 ====================
print("\n" + "=" * 70)
print("[2] 扁平 10-Class Classification")
print("=" * 70)

X_train_f, X_test_f, y_train_10, y_test_10 = train_test_split(
    X, y_original, test_size=0.3, random_state=42, stratify=y_original
)
...
rf_flat.fit(X_train_resampled_10, y_train_resampled_10)

y_pred_flat = rf_flat.predict(X_test_f)
...
plot_cm(cm_flat, TARGET_NAMES, f'RF Flat 10-Class (15 features)\nAccuracy: {acc_flat*100:.2f}%', 'rf_flat_10class_cm.png')

# ==================== Stage 1: 4分类 ====================
print("\n" + "=" * 70)
print("[3] Stage 1: 4-Class Severity Classification")
print("=" * 70)
...
plot_cm(cm_s1, SEVERITY_NAMES, 'RF Stage 1: 4-Class Severity (15 features)', 'rf_stage1_cm.png')

# ==================== Stage 2: 组内细分 ====================
print("\n" + "=" * 70)
print("[4] Stage 2: Intra-Group Sub-classification")
print("=" * 70)
...
for group_id in range(4):
    ...
    plot_cm(cm_sub, group_label_names,
            f'RF {GROUPS[group_id]["name"]} Group (15 features)',
            f'rf_stage2_g{group_id}_cm.png')
...
# ==================== 对比 ====================
print("\n" + "=" * 70)
print("对比: 新特征(39→15) vs 原始(30→20) vs 瞬态(30→21)")
print("=" * 70)
print(f"{'指标':<30} {'新特征RF':>12} {'原始RF':>12} {'瞬态RF':>12}")
print("-" * 68)
print(f"{'特征数':<30} {len(SELECTED_FEATURES):>12} {'20':>12} {'21':>12}")
print(f"{'Stage 1 Accuracy':<30} {acc_s1*100:>11.2f}% {'45.47%':>12} {'70.89%':>12}")
print(f"{'Overall 10-class':<30} {overall_acc:>11.2f}% {'58.87%':>12} {'69.59%':>12}")

# 保存结果
with open(os.path.join(output_dir, 'rf_new_features_results.txt'), 'w', encoding='utf-8') as f:
    f.write("=" * 70 + "\n")
    f.write("Random Forest — New 39 Features (15 selected)\n")
    f.write("=" * 70 + "\n\n")
    ...
    f.write(f"\nComparison:\n")
    f.write(f"  Original RF (30→20):  58.87%\n")
    f.write(f"  Transient RF (30→21): 69.59%\n")
    f.write(f"  New Features RF (39→{len(SELECTED_FEATURES)}): {overall_acc:.2f}%\n")

print(f"\n结果已保存至: {output_dir}/rf_new_features_results.txt")
print("=" * 70)
print("随机森林新特征分类完成!")
print("=" * 70)
