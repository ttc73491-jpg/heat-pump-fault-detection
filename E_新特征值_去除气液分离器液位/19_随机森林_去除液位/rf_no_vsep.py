...
# ==================== 数据加载 ====================
print("=" * 70)
print(f"随机森林 — 去除V_sep_liq 38特征数据集 ({len(SELECTED_FEATURES)} selected features)")
print("=" * 70)
...
plot_cm(cm_flat, TARGET_NAMES, f'RF Flat 10-Class ({len(SELECTED_FEATURES)} features, No V_sep_liq)\nAccuracy: {acc_flat*100:.2f}%', 'rf_flat_10class_cm.png')
...
plot_cm(cm_s1, SEVERITY_NAMES, f'RF Stage 1: 4-Class Severity ({len(SELECTED_FEATURES)} features, No V_sep_liq)', 'rf_stage1_cm.png')
...
for group_id in range(4):
    ...
    plot_cm(cm_sub, group_label_names,
            f'RF {GROUPS[group_id]["name"]} Group ({len(SELECTED_FEATURES)} features, No V_sep_liq)',
...
print("=" * 70)
print(f"{'指标':<30} {'去液位RF':>12} {'新特征RF':>12} {'原始RF':>12}")
print("-" * 68)
print(f"{'特征数':<30} {len(SELECTED_FEATURES):>12} {'15':>12} {'20':>12}")
print(f"{'Stage 1 Accuracy':<30} {acc_s1*100:>11.2f}% {'99.93%':>12} {'45.47%':>12}")
print(f"{'Overall 10-class':<30} {overall_acc:>11.2f}% {'99.50%':>12} {'58.87%':>12}")

with open(os.path.join(output_dir, 'rf_no_vsep_results.txt'), 'w', encoding='utf-8') as f:
    f.write("=" * 70 + "\n")
    f.write(f"Random Forest — 38 Features No V_sep_liq ({len(SELECTED_FEATURES)} selected)\n")
    f.write("=" * 70 + "\n\n")
    ...
    f.write(f"\nComparison:\n")
    f.write(f"  New Features RF (39→15):  99.50%\n")
    f.write(f"  Original RF (30→20):      58.87%\n")
    f.write(f"  No V_sep_liq RF (38→{len(SELECTED_FEATURES)}):  {overall_acc:.2f}%\n")

print(f"\n结果已保存至: {output_dir}/rf_no_vsep_results.txt")
print("=" * 70)
print("随机森林去除液位分类完成!")
print("=" * 70)
