...
| 算法 | 扁平10分类 | 分层10分类 | 备注 |
|------|-----------|-----------|------|
| **DNN 新特征 (15特征)** | **99.67%** | **★ 99.76%** | A8_Residual[128,128,128]，新最佳 |
| **RF 新特征 (15特征)** | 98.79% | **99.50%** | Stage 1 99.93% |
| **SVM 新特征 (15特征)** | 98.98% | **99.28%** | Stage 1 99.74% |
...
**核心发现：**
- **V_sep_liq[%] 是决定性特征：** 单特征 Gini 71.26%，几乎完美区分所有泄漏等级
- **新特征使问题基本解决：** 从 69.59% → 99.76%，提升 30.17 个百分点
- **三种算法均 > 99%：** DNN(99.76%) > RF(99.50%) > SVM(99.28%)，差距仅 0.48%
- **特征质量 > 一切：** 39→15 特征（含 V_sep_liq）直接将准确率从 ~60% 拉到 ~99.5%
- **架构不再重要：** 最差 DNN 架构也有 97.74%，10 种架构差距仅 1.93%（原始特征时全部坍缩在 23-25%）
