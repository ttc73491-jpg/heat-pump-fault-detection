"""
SVM故障程度细分分类脚本 - 热泵冷却系统故障检测
分层分类策略：正常/轻度/中度/重度 -> 组内10分类细分
"""
...
print("=" * 70)
print("SVM故障程度细分分类 - 热泵冷却系统故障检测")
print("=" * 70)

# 1. 加载数据
print("\n[1] 加载原始数据...")
data_dir = r'C:\Users\ccc\Desktop\algorithm\01_源数据'
output_dir = r'C:\Users\ccc\Desktop\algorithm\03_SVM'
...
X_scaled = scaler.fit_transform(combined_df[feature_cols].values)
...
X_train_scaled, X_test_scaled, _, _, _, _ = train_test_split(
...
# ============================================================
print("\n" + "=" * 70)
print("第一阶段：SVM 4分类 (正常/轻度/中度/重度)")
print("=" * 70)
...
print("\n[5] 训练SVM 4分类模型 (RBF核)...")
svm_4class = SVC(kernel='rbf', C=1.0, gamma='scale', random_state=42)
svm_4class.fit(X_train_resampled_scaled, y_train_resampled)

y_pred_sev = svm_4class.predict(X_test_scaled)
...
print("-" * 60)
print(f"\n  SVM 4分类总体准确率: {acc_4class*100:.2f}%")

plot_confusion_matrix(cm_4class, severity_labels,
    'SVM Confusion Matrix - 4-Class Classification\n(Normal/Light/Medium/Severe)',
    'svm_severity_4class_confusion_matrix.png', output_dir)

# ============================================================
# 第二阶段：分组内10分类
# ============================================================
print("\n" + "=" * 70)
print("第二阶段：SVM分组内10分类细分")
print("=" * 70)

correct_4class_mask = (y_pred_sev == y_test_sev)
print(f"\n[6] SVM 4分类正确的样本数: {np.sum(correct_4class_mask)} / {len(y_test_sev)}")
...
for group_id, group_info in groups.items():
    ...
    X_train_group_resampled_scaled = scaler_group.fit_transform(X_train_group_resampled)
    X_test_group_scaled = scaler_group.transform(X_test_group)

    svm_group = SVC(kernel='rbf', C=1.0, gamma='scale', random_state=42)
    svm_group.fit(X_train_group_resampled_scaled, y_train_group_resampled)

    y_pred_group_mapped = svm_group.predict(X_test_group_scaled)
    ...
    plot_confusion_matrix(cm_group, group_labels_names,
        f'SVM Confusion Matrix - {group_info["name"]} Group\n(10-class Sub-classification)',
        f'svm_severity_{group_id}_subclass_confusion_matrix.png', output_dir)

# ============================================================
# 结果汇总与对比
# ============================================================
print("\n" + "=" * 70)
print("SVM分组内10分类结果汇总")
print("=" * 70)
...
print(f"\n  SVM分组内10分类总体准确率: {overall_10class_accuracy:.2f}%")
...
# ============================================================
print("\n" + "=" * 70)
print("SVM vs 随机森林 对比")
print("=" * 70)

rf_comparison = pd.read_csv(r'C:\Users\ccc\Desktop\algorithm\01_源数据/classification_comparison.csv')

print("\n  分类方法                    SVM         随机森林")
print("-" * 60)
rf_4class_acc = float(rf_comparison[rf_comparison['分类方法']=='4分类']['准确率'].values[0].replace('%',''))
rf_10class_acc = float(rf_comparison[rf_comparison['分类方法']=='分组内10分类 (整体)']['准确率'].values[0].replace('%',''))
print(f"  4分类准确率                {acc_4class*100:.2f}%         {rf_4class_acc:.2f}%")
print(f"  分组内10分类准确率         {overall_10class_accuracy:.2f}%         {rf_10class_acc:.2f}%")

svm_comparison = pd.DataFrame({
    '分类方法': ['4分类', '分组内10分类 (整体)'],
    'SVM准确率': [f'{acc_4class*100:.2f}%', f'{overall_10class_accuracy:.2f}%'],
    '随机森林准确率': [f'{rf_4class_acc:.2f}%', f'{rf_10class_acc:.2f}%']
})
svm_comparison.to_csv(f'{output_dir}/svm_classification_comparison.csv', index=False)
print(f"\n对比结果已保存: {output_dir}/svm_classification_comparison.csv")
