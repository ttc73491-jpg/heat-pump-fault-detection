...
OUTPUT_DIR = r'C:\Users\ccc\Desktop\algorithm\回归算法\06_DNN_4features\output'
os.makedirs(OUTPUT_DIR, exist_ok=True)

# 4 hand-picked features (by column index in source CSV)
FEATURES = ['P_dis[bar]', 'T_dis[degC]', 'P_eva_out[bar]', 'T_air_out[degC]']
...
study = optuna.create_study(
    ...
    study_name='dnn_regression_4features'
)
...
ax.set_title(f'DNN (4 Features) -- True vs Predicted (Test R2={test_metrics["R2"]:.4f})', fontsize=14)
...
ax.set_title(f'DNN (4 Features) -- Prediction Error Distribution (SD={test_metrics["SD"]:.4f})', fontsize=14)
plt.tight_layout()
plt.savefig(os.path.join(OUTPUT_DIR, 'dnn_error_dist.png'), dpi=300, bbox_inches='tight')
plt.close()
print('Figure 2 saved: dnn_error_dist.png')

print('\nDNN regression (4 features) complete.')
